...
class Operador:
    ...
    def actualizar_precios_doble(self, credentials, progresswindow):
        self.crear_proveedores(credentials=credentials)
        self.crear_articulos()
        progresswindow.event_generate("<<SetMaxArticles>>", when="tail")
        self.sesionador.crear_sesiones(self.proveedores)
        ...
        print('Llegué :D')
        self.valuador.obtener_precios()
        maxiconsumo_sin_precio = []
        for articulo in self.articulos:
            if articulo.precio == 'Sin precio' and articulo.proveedor.nombre == 'MAXICONSUMO':
                articulo.proveedor = self.proveedores['OSCAR DAVID']
                maxiconsumo_sin_precio.append(articulo)
