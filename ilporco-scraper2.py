...
def boton():
    ...
    if busqueda.get() == False:
        ...
        hilo_operador = Thread(target=operador.inicializar, args=(admin_credenciales.obtener_credenciales(),), daemon=True)
        hilo_operador.start()
...
def abrir_sesion(proveedor:str):
    ...
    session_input.geometry('200x100')
    ttk.Entry(session_input, textvariable=sess_id_var).grid(row=0, column=0, pady=10, padx=10, sticky='NSEW')
    ttk.Button(session_input, command= lambda: operador.abrir_sesion(proveedor=proveedor,sess_id=sess_id_var), text='ABRIR SESIÓN').grid(row=1, column=0, pady=10, padx=10)
